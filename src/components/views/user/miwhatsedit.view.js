...
const MiWhatsEdit = () => {
    ...
    const validateURL = async (url, domain = "") => {
        let res = await UserService.getValidateURL(url, domain, params.id);
        if (res.success) {
            return false;
        } else {
            return true;
    ...
    const validate = async (fieldValues = data) => {
        ...
        if (('URL' in fieldValues || 'domain' in fieldValues) && !state.user.isTrial)
                    ...
                    <Grid item sm={12} lg={6} style={{ marginBlock: "10px", paddingInline: "15px", overflow: 'auto', maxHeight: "450px", display: 'flex', flexWrap: 'wrap' }}>
                        ...
                        <Grid item xs={12} className={classes.input}>
                            <Controls.Input
                                ...
                                readOnly={state.user.isTrial ? true : false}
                            />
                        </Grid>
