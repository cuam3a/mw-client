...
const arrayButtons = [
    ...
    { idButtonType: 2, name: 'WhatsApp', label: "Celular WhatsApp (incluir lada)", icon: <FontAwesomeIcon icon={faWhatsapp} />, type: "button", lada: "52" },
    { idButtonType: 3, name: 'Facebook Messenger', label: "Id Facebook", icon: <FontAwesomeIcon icon={faFacebookMessenger} />, type: "button" },
    { idButtonType: 4, name: 'Extra', label: "Link", icon: <FontAwesomeIcon icon={faLink} />, type: "extra" },
...
const MiWhatsAdd = () => {
    ...
    useEffect(() => {
        if (state.user.isTrial) {
            let validURL = false;
            while (!validURL) {
                const newURL = Global.getURLRandom();
                validURL = validateURL(newURL, data.domain);
                if (validURL) {
                    setData({ ...data, URL: Global.getURLRandom() });
                }
            }
    ...
    const handleInputChange = e => {
        ...
        } else if (e.target.name === "name" && !state.user.isTrial) {
            setData({
                ...data,
                [name]: value,
    ...
    const validate = async (fieldValues = data) => {
        ...
        if ('URL' in fieldValues || 'domain' in fieldValues)
            temp.URL = (await validateURL(fieldValues.URL, data.domain) && fieldValues.URL !== "" && fieldValues.URL !== null) ? "" : "URL ya existe."
    ...
    const validateURL = async (url, domain = "") => {
        let res = await UserService.getValidateURL(url, domain);
        if (res.success) {
            return false;
        } else {
            return true;
    ...
    const handleSave = async () => {
        ...
        if (await validate(data)) {
            let res = await UserService.addMiWhats(data);
            console.log(res);
            closeSnackbar();
            if (res.error) {
            ...
            <Paper className={classes.paperView}>
                <Grid container>
                    <Grid item xs={12} lg={12} style={{ flexGrow: 1 }}>
                        <Typography variant='h4' className={[classes.title, classes.textBold]}> Agregar MiWhats</Typography>
                    ...
                    <Grid item sm={12} lg={6} style={{ marginBlock: "10px", paddingInline: "15px", overflow: 'auto', maxHeight: "450px", display: 'flex', flexWrap: 'wrap' }}>
                        ...
                        <Grid xs={12} className={classes.input}>
                            <Controls.Input
                                ...
                                type="text"
                                value={data.URL}
                                onChange={(state.user.isTrial) ? () => { } : handleInputChange}
                                error={errors.URL}
                                start={
                                    <div style={{ maxWidth: '80%' }}>
                                        ...
                                        <Controls.Select
                                            ...
                                            size="small"
                                            onChange={(state.user.isTrial) ? () => { } : handleInputChange}
                                            options={(state?.user?.domains ?? [])}
                                        />
                                        {/* <Typography component="span" style={{ fontStyle:"italic", color:"#C4C4C4" }}>http://www.miwhats.app/
                                            </Typography> */}
                                    </div>
                                }
                            />
                                ...
                                return (
                                    <>
                                        <Grid container xs={12} style={{ marginBlock: "15px", marginInline: "5px", display: 'flex', flexWrap: 'wrap' }} key={i}>
                                            {
                                                ((item.type === "extra" || item.type === "video") && state.user.isTrial)
                                                    ? <></>
                                                    : <>
                                                        <Grid item xs={10}>
                                                            <Typography gutterBottom variant="span" style={{ fontSize: '15px', fontWeight: "bold" }}>
                                                                {item.icon}
                                                            </Typography>
                                                            <Typography gutterBottom variant="span" style={{ fontSize: '15px', fontWeight: "bold", marginLeft: "10px" }}>
                                                                {item.name}
                                                            </Typography>
...

                                            {
                                                (buttonExist && buttonExist.idState.toString() === "1" && (item.type === "button" || item.type === "extra" || item.type === "link"))
                                                    ...
                                                    (
                                                        <>
                                                            <Grid container xs={12} alignItems="center" style={{ marginBlock: "15px" }}>
                                                                {(item.type == "extra")
                                                                    ?
                                                                    <>
                                                                        <Grid item xs={4} >
                                                                            ...
                                                                            start={
                                                                                item.idButtonType == 2
                                                                                    ...
                                                                                    <div style={{ maxWidth: '80%' }}>
                                                                                        <FontAwesomeIcon icon={faLink} style={{ marginRight: '5px' }} />
                                                                                        <Controls.Select
                                                                                            value={(buttonExist) ? buttonExist.lada : "52"}
                                                                                            name="lada"
                                                                                            label=""
                                                                                            size="small"
                                                                                            onChange={(e) => handleInputChangeButton(e, { idButtonType: item.idButtonType, name: item.name, type: item.type })}
                        ...
                        <Grid xs={12}>
                            <Accordion style={{ border: "0.5px solid #CECECE", marginTop: "10px", marginBottom: "10px" }}>
                                ...
                                <AccordionDetails>
                                    ...
                                    <Grid xs={12} style={{ fontSize: '8px' }}>
                                        {
                                            arrayScript.map((item, i) => {
                                                const scriptExist = _.find(data.scripts, ['idScriptType', item.idScriptType])
                                                return (
                                                    <>
                                                        <Grid container alignItems="center" style={{ marginBlock: "10px", display: 'flex', flexWrap: 'wrap' }}>
                                                            <Grid item xs>
                                                                <Typography gutterBottom variant="span" style={{ fontSize: '15px', fontWeight: "bold" }}>
                                                                    {item.icon}
                                                                </Typography>
                                                        ...
                                                        {
                                                            (scriptExist && scriptExist.idState === "1")
                                                                ?
                                                                (
                                                                    <div>
                                                                        <Grid container alignItems="center" style={{ marginBlock: "10px" }}>
                    ...
                    <Tooltip title="Guardar Miwhats" aria-label="Guardar Miwhats">
                        <Fab className={classes.fab} color="secondary" aria-label="add" onClick={() => handleSave()} >
